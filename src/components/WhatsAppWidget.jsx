...
export default function WhatsAppWidget({ phone = '+2348130001122' }) {
  ...
  const openWhatsApp = () => {
    const encoded = encodeURIComponent(messages.filter((m) => m.from === 'user').map((m) => m.text).join('\n') || 'Hello')
      ...
      {open && (
        <div className="fixed bottom-20 right-6 z-[1200] w-[20rem] max-w-[94vw]">
          <div className="rounded-2xl border border-mssn-slate/10 bg-white shadow-lg">
            ...
            <div ref={listRef} className="max-h-64 space-y-2 overflow-auto p-3">
              {messages.map((m) => (
            ...
            <div className="flex items-center gap-2 border-t border-mssn-slate/10 p-3">
              <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') send() }} placeholder="Type a message..." className="flex-1 rounded-xl border border-mssn-slate/20 px-3 py-2 text-sm outline-none" />
              <button onClick={send} className="inline-flex items-center justify-center rounded-xl bg-mssn-green px-3 py-2 text-sm font-semibold text-white">Send</button>
            </div>
          </div>
        </div>
      )}
